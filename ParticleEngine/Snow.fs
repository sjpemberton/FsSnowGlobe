...
let gravity p = {X = 0.0; Y = 9.81 * 5.0}
...
let private tick delta state =
    ...
    let wind = fun _ -> { X = -(Math.Sin step * 0.5 + 0.5) * 80.0;
                          Y = Math.Sin(step * 1000.0)}
...
let private applyMoveForce points  =
    resetTimer()
    let p1, p2 = points
    moveForce <- fun _ -> {X = (p1.X - p2.X) * 200.0; Y = (p1.Y - p2.Y) * 200.0}

let private applyMouseForce = function
    | MouseEvent(LeftDown, currentPos) ->
        mouseForce <- repulse currentPos 20000.0 (fun d -> 1.0 / (1.0 + d))
